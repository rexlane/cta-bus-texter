...
class BusTracker
  ...
  def split_into_loc_and_dir(request)
    @location = "#{request.rpartition(' ').first} chicago"
    @direction = request.rpartition(' ').last.downcase
  ...
  def find_closest_stop(location, direction)
    ...
    @nearest_bus_stops = BusStop.near(location).where(dir: direction)
    # can allow for multiple routes at a single stop?
    @nearest_bus_stops.first
  end

  # def current_time
  #   CtaApiIntegration.new.current_time
  # end

  # def arrival_times(route, stop_id)
  #   CtaApiIntegration.new.arrival_times(route, stop_id)
  # end
  ...
  def bus(request)
    loc, dir = split_into_loc_and_dir(request)
