...
describe "texter_app" do
  ...
  describe "sorry" do
    it "says sorry" do
      expect(Parser.new.helpme).to eq(["Try texting:<br/>bus [location] [direction]<br/>map [location]<br/>define [word]<br/>", nil])
    end
  end

  describe "no_matching_method" do
    it "responds to bad method" do
      expect(Parser.new.no_matching_method).to eq(["Sorry, I don't understand. Try texting:<br/>bus [location] [direction]<br/>map [location]<br/>define [word]<br/>", nil])
    end
  end
